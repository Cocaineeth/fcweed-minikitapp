...
export default function Home()
{
    ...
    async function loadOwnedTokens(
        ...
        try {
            ...
            // NOTE(0xB800 Buffer): might consider storing in a database. the repeated calls to the rpc would eventually cause rate limiting
    ...
    const refreshStakingRunningRef = useRef(false);

    async function refreshStaking() {
        if (!stakingOpen) return;

        if (refreshStakingRunningRef.current) return;
        refreshStakingRunningRef.current = true;
        ...
        try {
            ...
            refreshStakingRunningRef.current = false;
    ...
    async function ensureCollectionApproval(
        ...
        if (!approved) {
            ...
            if (!tx) {
                throw new Error("User rejected approval transaction or wallet not available");
            }
            await waitForTx(tx);
        }
    }
    ...
    const pendingFloat = useMemo(
        () => (stakingStats ? parseFloat(stakingStats.pendingFormatted || "0") : 0),
        [stakingStats]
    );
    ...
    const pendingDisplay = useMemo(
        () =>
            {
                if (pendingFloat <= 0) return "0.00";
                if (pendingFloat >= 1_000_000)
                {
                    return `${(pendingFloat / 1_000_000).toFixed(2)}M`;
                }
                return pendingFloat.toFixed(2);
            },
        [pendingFloat]
    );

    const totalAvailable = useMemo(
        () => availablePlants.length + availableLands.length,
        [availablePlants, availableLands]
    );
