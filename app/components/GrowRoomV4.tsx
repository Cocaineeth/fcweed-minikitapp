...
export default function IsometricFarm({
    ...
    const TIERS: Record<string, { label: string; outerBg: string; innerBg: string; pattern: string; accent: string; border: string; glow: string; titleColor: string }> = {
        shed: {
            ...
            innerBg: "linear-gradient(180deg,#3a210e 0%,#5b3a1a 50%,#3a210e 100%)",
            pattern: "repeating-linear-gradient(90deg, rgba(0,0,0,0.30) 0 2px, transparent 2px 14px), repeating-linear-gradient(0deg, rgba(255,255,255,0.04) 0 1px, transparent 1px 24px)",
            accent: "#a06832",
            border: "4px solid #1f1006",
            glow: "0 0 40px rgba(160,104,50,0.40)",
            titleColor: "#f4d29c"
        },
        greenhouse: {
            label: "🌿 RARE GREENHOUSE",
            outerBg: "linear-gradient(180deg,#022c22 0%,#0f766e 50%,#022c22 100%)",
            innerBg: "linear-gradient(180deg,#042f2e 0%,#0f766e 45%,#022c22 100%)",
            pattern: "repeating-linear-gradient(45deg, rgba(34,211,238,0.10) 0 1px, transparent 1px 18px), repeating-linear-gradient(-45deg, rgba(34,211,238,0.10) 0 1px, transparent 1px 18px)",
            accent: "#22d3ee",
        ...
        mansion: {
            label: "👑 MYTHIC ESTATE",
            outerBg: "linear-gradient(180deg,#1a0633 0%,#7c3aed 50%,#1a0633 100%)",
            innerBg: "linear-gradient(180deg,#3b0764 0%,#7c3aed 30%,#a21caf 60%,#3b0764 100%)",
            pattern: "repeating-conic-gradient(from 0deg at 50% 50%, rgba(253,224,71,0.07) 0 4deg, transparent 4deg 16deg), repeating-linear-gradient(0deg, rgba(253,224,71,0.05) 0 1px, transparent 1px 32px)",
            accent: "#fde047",
            border: "4px solid #fbbf24",
            glow: "0 0 60px rgba(253,224,71,0.65)",
            titleColor: "#fde047"
        }
    };
    ...
    return (
        <div style={{
            position: "fixed", inset: 0, background: T.outerBg, zIndex: 1000,
            overflow: "hidden",
            ...
            <div style={{
                ...
                backgroundImage: `${T.pattern}, ${T.innerBg}`,
                display: "flex", flexDirection: "column",
                position: "relative", overflow: "hidden",
                ...(typeof window !== 'undefined' && window.innerWidth > 768 ? {
                    maxHeight: "92vh", borderRadius: 6, border: T.border,
                    boxShadow: T.glow
                } : { border: T.border })
            }}>
            <style>{`
